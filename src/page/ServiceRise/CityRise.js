...
export default class CityRise extends Component {
  maxWidth = 4000;
  ...
  handleSubmit = () => {
    const { personInfo, city, agentUserId, cardAArr, cardBArr, licenseArr } = this.state;
  ...
  // 表单校验
  handleCheckForm = () => {
    const { personInfo, isChecked, city, reg, cardAArr, cardBArr, licenseArr } = this.state;
  ...
  async getVerificationCode(mobile) {
    ...
    const res = await getCityCode(params);
    if (res && res.msg) {
      Toast.info(res.msg);
      this.init();
    }
  ...
  render() {
    const {
      ...
      cardBArr,
      licenseArr,
      personInfo: { nickName, mobile, companyName, verifyCode, career },
      isShowPercent,
      timePercent,
      isShowAgreement,
      authority,
    ...
    return (
      <div style={styles.container}>
        ...
        <div style={styles.areaContainer}>
          <div style={styles.inputBox}>
            <div style={styles.inputName}>公司名称</div>
            <input
              style={styles.inputValue}
              onChange={e => this.handleValueChange({ companyName: e.target.value.trim() })}
              placeholder="公司名称"
              value={companyName}
              maxLength={10}
            />
          ...
          <div style={styles.inputBox}>
            <div style={styles.inputName}>从事行业</div>
            <input
              style={styles.inputValue}
              onChange={e => this.handleValueChange({ career: e.target.value.trim() })}
              placeholder="输入从事行业"
              value={career}
              maxLength={10}
            />
