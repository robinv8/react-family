...
export default class OwnerRise extends Component {
  ...
  handleSubmit = () => {
    ...
    if (result) {
      ...
      if (city.length > 2) {
        params.areaId3 = Number(city[2].split('-')[0]);
        params.areaName = city[0].split('-')[1] + city[1].split('-')[1] + city[2].split('-')[1];
      }
  ...
  async getVerificationCode(mobile) {
    ...
    const res = await getBasicsCode(params);
    if (res && res.msg) {
      Toast.info(res.msg);
      this.init();
    }
